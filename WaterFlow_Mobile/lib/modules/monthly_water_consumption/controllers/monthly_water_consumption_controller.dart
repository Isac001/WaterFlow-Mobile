...
class MonthlyWaterConsumptionController extends GetxController {
  ...
  Future<List<ChartPoint>> fetchAndProcessGraphData(String id) async {
    ...
    final List<ChartPoint> chartPoints = weeklyDetails.map((detail) {
      // Ensure that the data is not null before processing.
      if (detail.startDate == null || detail.endDate == null || detail.consumption == null) {
        return null; // Return null to be filtered out later.
      }

      // Format the dates for the X-axis label.
      final String startDateFormatted = DateFormat('dd/MM').format(detail.startDate!);
      final String endDateFormatted = DateFormat('dd/MM').format(detail.endDate!);
    ...
    }).where((point) => point != null).cast<ChartPoint>().toList(); // Filter out any resulting null values.
  ...
  Future<void> lazyLoad(int pageKey) async {
    try {
      ...
      final response = await _serivce.listWeeklyWaterConsumption(pageKey,
          showLoading: false);
